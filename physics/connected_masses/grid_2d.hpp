...
/**
 * @brief Generate edge list for a 2D triangular grid
 *
 * This function creates the connectivity pattern for a 2D triangular mesh where:
 * - Each rectangular cell is split into two triangles by diagonal connections
 * - This creates a very stable structure suitable for cloth-like simulations
 * - Uses alternating diagonal pattern for uniform triangulation
 *
 * The grid is indexed row-major: index = row * cols + col
 *
 * Example 3x3 triangular grid:
 * 0 --1-- 2
 * |\ /|\ /|
 * | X | X |
 * |/ \|/ \|
 ...
 * 6 --7-- 8
 *
 * Each cell gets two diagonals forming an 'X' pattern, creating 4 triangles per cell.
 * This provides excellent structural stability.
 *
 * @param rows Number of rows in the grid (must be >= 2)
 * @param cols Number of columns in the grid (must be >= 2)
 * @return Vector of edge pairs (i, j) where i < j
 ...
 */
[[nodiscard]] inline std::vector<std::pair<size_t, size_t>>
makeTriangularGridEdges(size_t rows, size_t cols) {
    if (rows < 2) {
        throw std::invalid_argument("Grid must have at least 2 rows (got " +
                                    std::to_string(rows) + ")");
    }
    ...
    // Horizontal edges (same as quad grid)
    for (size_t r = 0; r < rows; ++r) {
        for (size_t c = 0; c < cols - 1; ++c) {
            edges.push_back({index(r, c), index(r, c + 1)});
        }
    ...
    // Vertical edges (same as quad grid)
    for (size_t r = 0; r < rows - 1; ++r) {
        for (size_t c = 0; c < cols; ++c) {
            edges.push_back({index(r, c), index(r + 1, c)});
        }
    ...
    // Diagonal edges - create full triangulation with 'X' pattern
    // This adds BOTH diagonals in each cell for maximum stability
    for (size_t r = 0; r < rows - 1; ++r) {
        for (size_t c = 0; c < cols - 1; ++c) {
            // Main diagonal (top-left to bottom-right)
            edges.push_back({index(r, c), index(r + 1, c + 1)});
            // Anti-diagonal (top-right to bottom-left)
            edges.push_back({index(r, c + 1), index(r + 1, c)});
        }
    }
...
/**
 * @brief Compile-time version for creating constexpr triangular edge arrays
 *
 * Usage:
 * @code
 * constexpr auto edges = makeTriangularGridEdgesArray<3, 3>();
 * @endcode
 */
template<size_t Rows, size_t Cols>
constexpr auto makeTriangularGridEdgesArray() {
    static_assert(Rows >= 2, "Grid must have at least 2 rows");
    static_assert(Cols >= 2, "Grid must have at least 2 columns");
    ...
    constexpr size_t horizontal_edges = Rows * (Cols - 1);
    constexpr size_t vertical_edges = (Rows - 1) * Cols;
    constexpr size_t diagonal_edges = 2 * (Rows - 1) * (Cols - 1);  // Both diagonals per cell
    constexpr size_t total_edges = horizontal_edges + vertical_edges + diagonal_edges;
    ...
    // Diagonal edges - 'X' pattern for full triangulation
    for (size_t r = 0; r < Rows - 1; ++r) {
        for (size_t c = 0; c < Cols - 1; ++c) {
            // Main diagonal
            edges[edge_idx++] = {index(r, c), index(r + 1, c + 1)};
            // Anti-diagonal
            edges[edge_idx++] = {index(r, c + 1), index(r + 1, c)};
        }
    }
